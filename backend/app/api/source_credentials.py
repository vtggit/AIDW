...
def create_source_credential(
    ...
    return service.create_source_credential(payload)
...
def update_source_credential(
    ...
    entity = service.update_source_credential(entity_id, payload)
    if entity is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"SourceCredential '{entity_id}' not found.",
...
def delete_source_credential(
    ...
    if not service.delete_source_credential(entity_id):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"SourceCredential '{entity_id}' not found.",
        )

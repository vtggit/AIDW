...
class DatasetPostgresRepository:
    """PostgreSQL repository for the datasets table."""
    ...
    def get_by_id(self, entity_id: str) -> dict | None:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM datasets WHERE id = %s", (entity_id,))
            row = cur.fetchone()
    ...
    def create(self, data: dict, actor: str | None = None) -> dict:
        ...
        with get_cursor() as cur:
            cur.execute(
                "INSERT INTO datasets (id, name, object_type, source_id, first_seen_run_id, last_seen_run_id, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                (
                    new_id,
                    data.get("name"),
            ...
            if (
                ...
                record_audit(cur, actor, "create", "datasets", new_id)
    ...
    def update(
        ...
        with get_cursor() as cur:
            cur.execute(
                f"UPDATE datasets SET {', '.join(set_clauses)} WHERE id = %s",
                values + [entity_id],
            )
    ...
    def delete(self, entity_id: str, actor: str | None = None) -> bool:
        with get_cursor() as cur:
            cur.execute("DELETE FROM datasets WHERE id = %s", (entity_id,))
            deleted = cur.rowcount > 0
            if actor and deleted:
                record_audit(cur, actor, "delete", "datasets", entity_id)
            return deleted

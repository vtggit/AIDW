"""PostgreSQL repository for field_profiles."""
...
class FieldProfilePostgresRepository:
    """PostgreSQL repository for the field_profiles table."""

    def list_all(self) -> list[dict]:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM field_profiles ORDER BY created_at DESC")
            return [_row_to_dict(r) for r in cur.fetchall()]

    def get_by_id(self, entity_id: str) -> dict | None:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM field_profiles WHERE id = %s", (entity_id,))
            row = cur.fetchone()
    ...
    def update(self, entity_id: str, data: dict) -> dict | None:
        ...
        with get_cursor() as cur:
            cur.execute(
                f"UPDATE field_profiles SET {', '.join(set_clauses)} WHERE id = %s",
                values + [entity_id],
            )
        return self.get_by_id(entity_id)
    ...
    def delete(self, entity_id: str) -> bool:
        with get_cursor() as cur:
            cur.execute("DELETE FROM field_profiles WHERE id = %s", (entity_id,))

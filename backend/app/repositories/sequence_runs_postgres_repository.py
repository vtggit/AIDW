"""PostgreSQL repository for sequence_runs."""
...
class SequenceRunPostgresRepository:
    ...
    def get_by_id(self, entity_id: str) -> dict | None:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM sequence_runs WHERE id = %s", (entity_id,))
            row = cur.fetchone()
    ...
    def update(self, entity_id: str, data: dict) -> dict | None:
        ...
        with get_cursor() as cur:
            cur.execute(
                f"UPDATE sequence_runs SET {', '.join(set_clauses)} WHERE id = %s",
                values + [entity_id],
            )
        return self.get_by_id(entity_id)
    ...
    def delete(self, entity_id: str) -> bool:
        with get_cursor() as cur:
            cur.execute("DELETE FROM sequence_runs WHERE id = %s", (entity_id,))

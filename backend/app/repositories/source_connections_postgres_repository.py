"""PostgreSQL repository for source_connections."""
...
class SourceConnectionPostgresRepository:
    """PostgreSQL repository for the source_connections table."""

    def list_all(self) -> list[dict]:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM source_connections ORDER BY created_at DESC")
            return [_row_to_dict(r) for r in cur.fetchall()]

    def get_by_id(self, entity_id: str) -> dict | None:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM source_connections WHERE id = %s", (entity_id,))
            row = cur.fetchone()

"""PostgreSQL repository for dashboards."""
...
class DashboardPostgresRepository:
    """PostgreSQL repository for the dashboards table."""

    def list_all(self) -> list[dict]:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM dashboards ORDER BY created_at DESC")
            return [_row_to_dict(r) for r in cur.fetchall()]

    def get_by_id(self, entity_id: str) -> dict | None:
        with get_cursor() as cur:
            cur.execute("SELECT * FROM dashboards WHERE id = %s", (entity_id,))
            row = cur.fetchone()
    ...
    def update(self, entity_id: str, data: dict) -> dict | None:
        ...
        with get_cursor() as cur:
            cur.execute(
                f"UPDATE dashboards SET {', '.join(set_clauses)} WHERE id = %s",
                values + [entity_id],
            )
        return self.get_by_id(entity_id)
    ...
    def delete(self, entity_id: str) -> bool:
        with get_cursor() as cur:
            cur.execute("DELETE FROM dashboards WHERE id = %s", (entity_id,))

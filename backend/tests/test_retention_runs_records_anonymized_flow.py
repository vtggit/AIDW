"""Field-flow test — records_anonymized (CHECK >= 0) round-trips and rejects out-of-range."""


def test_retention_runs_records_anonymized_round_trips(client, admin_headers):
    body = {"name": "test"}
    body["records_anonymized"] = 0
    ...
    assert row.get("records_anonymized") == 0
    bad_body = {"name": "test"}
    bad_body["records_anonymized"] = -1
    ...
    assert all(
        r.get("records_anonymized") != -1 for r in after.json()

"""Field-flow test — source_id (FK -> sources) round-trips through the connection_tests API."""


def test_connection_tests_source_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    rejected = client.post("/api/connection-tests", json=bogus, headers=admin_headers)

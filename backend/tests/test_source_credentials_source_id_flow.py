"""Field-flow test — source_id (FK -> sources) round-trips through the source_credentials API."""


def test_source_credentials_source_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    rejected = client.post("/api/source-credentials", json=bogus, headers=admin_headers)

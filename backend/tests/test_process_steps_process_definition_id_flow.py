"""Field-flow test — process_definition_id (FK -> process_definitions) round-trips through the process_steps API."""


def test_process_steps_process_definition_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    rejected = client.post("/api/process-steps", json=bogus, headers=admin_headers)

"""Field-flow test — dataset_id (FK -> datasets) round-trips through the deletion_requests API."""


def test_deletion_requests_dataset_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    created = client.post("/api/deletion-requests", json=body, headers=admin_headers)
    ...
    rejected = client.post("/api/deletion-requests", json=bogus, headers=admin_headers)

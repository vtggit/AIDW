"""Field-flow test — grid_col_start (CHECK > 0) round-trips and rejects out-of-range."""


def test_dashboard_item_layouts_grid_col_start_round_trips(client, admin_headers):
    body = {"name": "test"}
    body["grid_col_start"] = 1
    created = client.post(
    ...
    assert row.get("grid_col_start") == 1
    bad_body = {"name": "test"}
    bad_body["grid_col_start"] = 0
    ...
    assert all(
        r.get("grid_col_start") != 0 for r in after.json()

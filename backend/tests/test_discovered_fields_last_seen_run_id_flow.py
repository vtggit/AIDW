"""Field-flow test — last_seen_run_id (FK -> discovery_runs) round-trips through the discovered_fields API."""


def test_discovered_fields_last_seen_run_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    body = {"name": "test"}
    body["last_seen_run_id"] = parent_id
    ...
    assert row.get("last_seen_run_id") == parent_id
    bogus = {"name": "test"}
    bogus["last_seen_run_id"] = "no-such-parent-id"

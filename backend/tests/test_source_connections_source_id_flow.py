"""Field-flow test — source_id (FK -> sources) round-trips through the source_connections API."""


def test_source_connections_source_id_round_trips(client, admin_headers):
    parent = client.post(
    ...
    rejected = client.post("/api/source-connections", json=bogus, headers=admin_headers)

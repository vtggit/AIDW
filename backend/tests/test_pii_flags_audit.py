"""Audit-hook test — pii_flags mutations write the audit trail (same transaction)."""

import uuid


def test_pii_flags_mutations_are_audited(client, admin_headers):
    created = client.post(
        "/api/pii-flags",
    ...
    dele = client.delete(f"/api/pii-flags/{eid}", headers=admin_headers)
    ...
    assert all(r["entity_type"] == "pii_flags" and r["actor"] for r in rows)
